...
/**
 * A component that allows users to upload images containing math problems and receive step-by-step solutions.
...
const Solve = () => {
  ...
  const handleSolveProblem = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/gemini/mathematics/solve", {
        method: "POST",
        body: formData,
      });
  ...
  const handleExport = async (type: "pdf" | "image") => {
    ...
    try {
      ...
      if (type === "pdf" && pdf) {
        pdf.save("math-solutions.pdf");
        toast.success("PDF exported successfully");
      } else if (type === "image" && zip) {
        const content = await zip.generateAsync({ type: "blob" });
        const link = document.createElement("a");
        link.href = URL.createObjectURL(content);
        link.download = "math-solutions.zip";
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Upload images containing math problems and get detailed, step-by-step
          solutions.
        </p>
      </div>
